...
__global__ void kernel1(float* aptr, float* bptr, float* cptr, int n){
    Tensor a = make_tensor(aptr, make_layout(make_shape(n), make_stride(1)));
    Tensor b = make_tensor(bptr, make_layout(make_shape(n), make_stride(1)));
    Tensor c = make_tensor(cptr, make_layout(make_shape(n), make_stride(1)));
...
int main(){
    ...
    {
        const int n = 32;
        torch::Tensor ta = torch::arange(n, torch::dtype(torch::kFloat32));
        torch::Tensor tb = torch::arange(n, torch::dtype(torch::kFloat32));
        torch::Tensor tc = torch::empty (n, torch::dtype(torch::kFloat32));
