...
__global__ void cute_gemm_multi_stage_kernel(void *Dptr, const void *Aptr, const void *Bptr, int m, int n, int k) {
  using namespace cute;using namespace gemm_config;
...
torch::Tensor cute_gemm_multi_stage(torch::Tensor A, torch::Tensor B){
    ...
    cute_gemm_multi_stage_kernel<<<grid, block, kShmSize>>>(C.data_ptr(), A.data_ptr(), B.data_ptr(),M, N, K);
    return C;
}
...
void test(torch::Tensor a, torch::Tensor b, std::string name){
    ...
    float eps = 1e-1;
    if (a.allclose(b, eps, eps)) std::cout << name << ": pass" << std::endl;
    else std::cout << name << ": fail" << std::endl;
}
int main(){
    const int M = 1024;const int N = 1024;const int K = 256;
    ...
    auto o = cute_gemm_multi_stage(A, B);

    auto o_ref = torch::matmul(A, B.transpose(0, 1));

    test(o, o_ref, "cute_gemm_multi_stage");
    return 0;
}