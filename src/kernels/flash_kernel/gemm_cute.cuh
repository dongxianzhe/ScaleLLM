...
cute_gemm_multi_stage_demo_kernel(void *Dptr, const void *Aptr, const void *Bptr, int m, int n,
                 int k) {
  using namespace cute;
  using X = Underscore;
...
torch::Tensor cute_gemm_multi_stage_demo(torch::Tensor A, torch::Tensor B){
  torch::Tensor B_kmajor = B.transpose(0, 1).contiguous();
  const int M = A.size(0);
  const int K = A.size(1);
  const int N = B_kmajor.size(0);
  ...
  cudaFuncSetAttribute(cute_gemm_multi_stage_demo_kernel<decltype(gemm_config)>,
                        cudaFuncAttributeMaxDynamicSharedMemorySize, shm_size);
  cute_gemm_multi_stage_demo_kernel<decltype(gemm_config)><<<grid, block, shm_size>>>(
    C.data_ptr(),
    A.data_ptr(),
    B_kmajor.data_ptr(),
    M, N, K);
